...
T const lengthZ = 0.05; //orig 0.05
...
T centerY = lengthY*.5;
T centerZ = lengthZ*.9;
T const sphereDensity = 2500;
T const sphereDiameter = 0.0025;
Vector<T,3> sphereCenter = {centerX,centerY,centerZ};
//Vector<T,3> cubeOrientation = {0.,15.,0.};
Vector<T,3> sphereVelocity = {0.,0.,0.};
Vector<T,3> externalAcceleration = {.0, .0, -9.81 * (1. - physDensity / sphereDensity)}; //gravity - buoyancy

// Characteristic Quantities
T const charPhysLength = lengthX/2.; //was lengthX
T const charPhysVelocity = 0.0115;    // Assumed maximal velocity
...
int main(int argc, char* argv[])
{
  ...
  //Sphere indicator
  SmoothIndicatorSphere3D<T, T, true> particleIndicator (sphereCenter, 0.5*sphereDiameter, epsilon, sphereDensity, sphereVelocity);
